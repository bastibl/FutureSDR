...
#[derive(Clone, Debug)]
pub struct GipsScheduler {
    inner: Arc<GipsSchedulerInner>,
}

struct GipsSchedulerInner {
    executor: Arc<GipsExecutor>,
    workers: Vec<(thread::JoinHandle<()>, oneshot::Sender<()>)>,
}

impl fmt::Debug for GipsSchedulerInner {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GipsSchedulerInner").finish()
    }
}

impl Drop for GipsSchedulerInner {
    fn drop(&mut self) {
...
impl GipsScheduler {
    /// Create Gips scheduler
    pub fn new() -> GipsScheduler {
        let executor = Arc::new(GipsExecutor::new());
        ...
        GipsScheduler {
            inner: Arc::new(GipsSchedulerInner { executor, workers }),
        }
    }
...
impl Scheduler for GipsScheduler {
    fn run_topology(
        &self,
            ...
            if block.is_blocking() {
                ...
                self.inner
                    ...
                    .spawn_executor(
                        ...
                        GipsScheduler::map_block(id, n_blocks, n_cores),
                    )
                    .detach();
            } else {
                self.inner
...
impl Default for GipsScheduler {
    fn default() -> Self {
        Self::new()
    }
}
...
///     }));
/// ```
pub struct GipsExecutor {
    /// The executor state.
    state: once_cell::sync::OnceCell<Arc<State>>,
}

impl UnwindSafe for GipsExecutor {}
impl RefUnwindSafe for GipsExecutor {}
