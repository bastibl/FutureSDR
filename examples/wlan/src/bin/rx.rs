...
fn main() -> Result<()> {
    ...
    fg.connect_dyn(prev.dyn_stream_output(output)?, delay.dyn_stream_input("input")?)?;

    let complex_to_mag_2 = fg.add(Apply::<_, _, _>::new(|i: &Complex32| i.norm_sqr()))?;
    let float_avg = MovingAverage::<f32>::new(64);
    fg.connect_dyn(prev.dyn_stream_output(output)?, complex_to_mag_2.dyn_stream_input("input")?)?;
    ...
    fg.connect_dyn(prev.dyn_stream_output(output)?, mult_conj.dyn_stream_input("in0")?)?;
